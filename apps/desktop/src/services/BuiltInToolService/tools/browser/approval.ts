...
function approval(command: string, reason: string, description = ''): ToolApprovalRequest {
    return {
        title: tt('浏览器操作确认'),
        description,
        command: truncateText(command, 180),
        riskLabel: '',
        reason,
        commandLabel: '',
        approveLabel: tt('批准'),
...
export async function createBrowserApprovalRequest(
    ...
    if (toolId === 'browser_session') {
        if (operation === 'start' || operation === 'stop') {
            return approval(operation, tt('此操作会启动或停止浏览器自动化会话。'));
        }
    ...
    if (operation === 'navigate') {
        ...
        return approval(`navigate ${targetUrl}`, tt('此操作会让浏览器打开或切换到新的网页。'));
    }
    ...
    if (operation === 'fill_form') {
        return approval(`fill_form ${formatTarget(args)}`, tt('此操作会向网页表单输入一组内容。'));
    }

    if (operation === 'type') {
        return approval(`type ${formatTarget(args)}`, tt('此操作会在网页中输入文本。'));
    }
