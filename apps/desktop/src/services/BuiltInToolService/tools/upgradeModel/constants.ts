...
export const UPGRADE_MODEL_TOOL_INPUT_SCHEMA: AiToolDefinition['input_schema'] = {
    ...
    properties: {
        role: {
            ...
            description:
                'Optional configured model role. Use "fast" for simple tasks, "general" for complex tasks without a matching scenario, or "entry" to return to the entry model.',
        },
        scenario: {
            type: ['string', 'null'],
            minLength: 1,
            pattern: '\\S',
            description:
                'Optional scenario name from the configured model preferences. Use null to restore the entry model.',
        },
        restore: {
            type: 'boolean',
            description: 'Set true to switch back to the entry model.',
        },
    },
    required: [],
