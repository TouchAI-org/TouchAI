...
const FILE_SYSTEM_TEST_TIMEOUT_MS = 60_000;
...
describe('normalizeVelopackReleaseAssets', () => {
    it(
        'renames current beta packages and MSI while dropping setup and portable artifacts',
        async () => {
            ...
            try {
                ...
                const assets = JSON.parse(
                    await readFile(join(releaseDir, 'assets.beta.json'), 'utf8')
                );
                ...
                await expect(readFile(join(releaseDir, renamedDelta), 'utf8')).resolves.toBe(
                    'delta'
                );
                ...
                await expect(
                    readFile(join(releaseDir, currentSetup), 'utf8')
                ).rejects.toMatchObject({
                    code: 'ENOENT',
                });
                await expect(
                    readFile(join(releaseDir, currentPortable), 'utf8')
                ).rejects.toMatchObject({
                    code: 'ENOENT',
                });
        ...
        FILE_SYSTEM_TEST_TIMEOUT_MS
    );

    it(
        'omits the stable channel from public artifact names',
        async () => {
            const normalizeVelopackReleaseAssets = await loadNormalizer();
            const { root, releaseDir } = await createFixture();
