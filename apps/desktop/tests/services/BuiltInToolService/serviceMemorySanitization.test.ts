...
describe('builtInToolService memory sanitization', () => {
    ...
    it('redacts memory content even when malformed upsert args fail parsing', async () => {
        const events: ToolEvent[] = [];
        const secret = 'OPENAI_API_KEY=sk-test-secret-value';
    ...
    it('redacts secret-like memory titles from rejected call_start semantics', async () => {
        const events: ToolEvent[] = [];
        const secretTitle = 'OPENAI_API_KEY=sk-title-secret-value';

        const result = await builtInToolService.executeTool({
            toolCall: {
                id: 'call-secret-title',
                name: 'builtin__memory',
                arguments: '',
            },
            toolArgs: {
                ...
                title: secretTitle,
                applicability: 'When calling external services.',
                content: 'Use normal project context.',
            },
            iteration: 0,
            currentModel: model,
            hasExecutedBuiltInTool: () => false,
        ...
        expect(JSON.stringify(events)).not.toContain(secretTitle);
        const callStart = events.find((event) => event.type === 'call_start');
        expect(callStart?.type).toBe('call_start');
        if (!callStart || callStart.type !== 'call_start') {
            throw new Error('Expected call_start event');
        }
        expect(callStart.arguments.title).toBe('[REDACTED_MEMORY_SECRET]');
        expect(callStart.builtinConversationSemantic?.target).toBe('[REDACTED_MEMORY_SECRET]');
    });

    it('redacts unknown secret-like fields from malformed memory args before events', async () => {
        const events: ToolEvent[] = [];
        const apiKey = 'OPENAI_API_KEY=sk-unknown-memory-secret';
        const token = 'ghp_1234567890abcdefghijklmnopqrstuvwxyzAB';

        const result = await builtInToolService.executeTool({
            toolCall: {
                id: 'call-malformed-unknown-memory-secret',
                name: 'builtin__memory',
                arguments: '',
            },
            toolArgs: {
        ...
        expect(callStart.arguments.api_key).toBe('[REDACTED_SECRET_LIKE_CONTENT]');
        expect(callStart.arguments.nested).toEqual({
    ...
    it('redacts unknown secret-like fields from malformed search args before events and logs', async () => {
        const events: ToolEvent[] = [];
        const token = 'ghp_1234567890abcdefghijklmnopqrstuvwxyzAB';
        const apiKey = 'OPENAI_API_KEY=sk-unknown-search-secret';

        const result = await builtInToolService.executeTool({
            toolCall: {
                id: 'call-malformed-unknown-search-secret',
                name: 'builtin__search_conversation',
                arguments: '',
            },
            toolArgs: {
        ...
        expect(callStart.arguments.token).toBe('[REDACTED_SECRET_LIKE_CONTENT]');
        expect(callStart.arguments.nested).toEqual({
            api_key: '[REDACTED_SECRET_LIKE_CONTENT]',
        });

        const createLogCalls = vi.mocked(createBuiltInToolLog).mock.calls;
        const logInput = createLogCalls[createLogCalls.length - 1]?.[0].input ?? '';
        expect(logInput).not.toContain(token);
        expect(logInput).not.toContain(apiKey);
        expect(logInput).toContain('[REDACTED_SECRET_LIKE_CONTENT]');
    });
    ...
    it('sanitizes built-in tool call arguments before they enter assistant history', async () => {
        ...
        const malformedSearch = await builtInToolService.sanitizeToolCallForHistory({
            id: 'call-history-malformed-search',
            name: 'builtin__search_conversation',
            arguments: `{"query":"${githubToken}"`,
        });
        ...
        const serialized = JSON.stringify([sanitizedSearch, sanitizedMemory, malformedSearch]);
        expect(serialized).not.toContain(githubToken);
        expect(serialized).not.toContain(awsAccessKey);
        expect(serialized).not.toContain(memorySecret);
        expect(malformedSearch.arguments).toContain('[REDACTED_SECRET_LIKE_CONTENT]');
    });

    it('redacts unresolved built-in tool call arguments before assistant history', async () => {
        const githubToken = 'ghp_1234567890abcdefghijklmnopqrstuvwxyzAB';
        const durablePreference = 'Prefer the right monitor for meeting notes.';
        vi.mocked(findBuiltInToolByToolId).mockResolvedValueOnce(undefined);

        const sanitized = await builtInToolService.sanitizeToolCallForHistory({
            id: 'call-history-disabled-search',
            name: 'builtin__search_conversation',
            arguments: JSON.stringify({
                query: githubToken,
                note: durablePreference,
            }),
        });

        expect(sanitized.arguments).not.toContain(githubToken);
        expect(sanitized.arguments).not.toContain(durablePreference);
        expect(sanitized.arguments).toContain('[REDACTED_SECRET_LIKE_CONTENT]');
    });
});
